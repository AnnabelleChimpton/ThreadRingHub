...
export async function ringManagementRoutes(fastify: FastifyInstance) {
    ...
    /**
     * PUT /trp/rings/:slug/members/:actorDid - Update member role
     */
    fastify.put<{
        ...
        try {
            ...
            if (!['member', 'moderator'].includes(role)) {
                return reply.code(400).send({
                    error: 'Invalid role',
                    message: 'Role must be either "member" or "moderator"'
                });
            }

            // Get ring with memberships and roles
            const ring = await prisma.ring.findUnique({
                where: { slug },
                include: {
                    memberships: {
            ...
            if (!requestorMembership || requestorMembership.role?.name !== 'owner') {
                return reply.code(403).send({
                    error: 'Forbidden',
                    message: 'Only ring owner can change member roles'
                });
            }
            ...
            // Get role ID
            const roleRecord = ring.roles.find(r => r.name === role);
            if (!roleRecord) {
                return reply.code(400).send({
                    error: 'Role not found',
                    message: `Role "${role}" not found in ring`
                });
            }
